...
reqmap gaps --snapshot  「ここまでは見た」を記録する
...
FSL（jssm互換）で状態遷移を書くと、**二重送信・二重決済・例外経路の欠落**が
状態×イベントの網羅から出ます。
...
| 二重送信・二重決済・返金経路なし | `fsm.state_event_hole` / `fsm.unused_event` |
...
scripts/reqmap-cli trace  --root=examples/takeout-app   # 決定と設計の追跡
scripts/reqmap-cli view   --root=examples/takeout-app
```

`examples/takeout-app/sources/20260915_定例.md` が議事録、
`_changesets/20260915-teirei.yml` が抽出結果です。
...
| `coverage-grids` | 網羅グリッドや FSL を作る・直す・調整するとき |
